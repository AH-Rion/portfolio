      ...
      <div className="max-w-4xl mx-auto">
        <motion.div ref={ref} initial="hidden" animate={isInView ? "show" : "hidden"} variants={staggerContainer}>
          <motion.div variants={fadeInUp} className="text-center mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-bold">What <span className="gradient-text">Clients</span> Say</h2>
          </motion.div>
          <motion.div variants={fadeInUp} className="relative">
            <div className="glass rounded-2xl p-8 md:p-12 glow-box relative overflow-hidden min-h-[280px] flex items-center">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={current} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="text-center relative z-10 w-full">
                  ...
                  <p className="text-lg md:text-xl text-foreground/90 leading-relaxed mb-8 italic">"{testimonials[current].text}"</p>
            ...
            <div className="flex justify-center items-center gap-4 mt-6">
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={prev} className="p-2 rounded-full glass text-muted-foreground hover:text-foreground"><ChevronLeft size={20} /></motion.button>
              ...
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={next} className="p-2 rounded-full glass text-muted-foreground hover:text-foreground"><ChevronRight size={20} /></motion.button>
            </div>
          </motion.div>
        </motion.div>
      </div>
